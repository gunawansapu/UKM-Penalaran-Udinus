...
const Footer = () => {
  ...
  useEffect(() => {
    ...
    // Jangan fetch kalau lagi di halaman admin
  ...
  const formatWaNumber = (numberString) => {
    if (!numberString) return '';
    // Hapus karakter non-digit
    const cleaned = numberString.replace(/\D/g, '');
    // Asumsikan format awalnya 62...
    if (cleaned.startsWith('62')) {
      const parts = [
        '+62',
        cleaned.slice(2, 5), // 831
        cleaned.slice(5, 9), // 0715
        cleaned.slice(9)     // 4446
      ];
      return parts.filter(Boolean).join(' ').replace(/ (\d{4}) (\d{4})/, ' $1-$2'); // +62 831-0715-4446
    }
    return numberString; // Kembalikan default jika bukan 62
  };
