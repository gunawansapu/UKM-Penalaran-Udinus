...
const EvenCard = ({ 
  ...
  const handleRegistration = () => {
    if (registrationUrl) window.open(registrationUrl, '_blank');
  };
  ...
  const handleClosedRegistration = () => {
    ...
    modal.innerHTML = `
      <div class="bg-white rounded-2xl p-8 max-w-md mx-4 text-center shadow-2xl transform">
        ...
        <button onclick="this.parentElement.parentElement.remove()" class="bg-red-500 text-white px-6 py-2 rounded-full hover:bg-red-600 transition-colors">Mengerti</button>
      </div>
    `;
    document.body.appendChild(modal);
    setTimeout(() => { if (document.body.contains(modal)) document.body.removeChild(modal); }, 4000);
  };
  ...
  const handleCompletedEvent = () => {
    ...
    modal.innerHTML = `
      <div class="bg-white rounded-2xl p-8 max-w-md mx-4 text-center shadow-2xl transform">
        ...
        <button onclick="this.parentElement.parentElement.remove()" class="bg-green-500 text-white px-6 py-2 rounded-full hover:bg-green-600 transition-colors">Oke</button>
      </div>
    `;
    document.body.appendChild(modal);
    setTimeout(() => { if (document.body.contains(modal)) document.body.removeChild(modal); }, 4000);
  ...
  const getButtonConfig = () => {
    if (isEventCompleted) {
      return {
        ...
        icon: <svg className="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
      };
    }
    if (isRegistrationClosed) {
      return {
        ...
        icon: <svg className="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M13.477 14.89A6 6 0 015.11 6.524l8.367 8.368zm1.414-1.414L6.524 5.11a6 6 0 018.367 8.367zM18 10a8 8 0 11-16 0 8 8 0 0116 0z" clipRule="evenodd" /></svg>
      };
    }
    if (registrationUrl && isRegistrationOpen) {
      return {
        ...
        icon: <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
      };
    }
    return null;
  };
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-lg hover:shadow-xl overflow-hidden border border-gray-100 transition-all duration-300 flex flex-col h-full ${className}`}>
      {/* Image Section */}
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className="space-y-2 mb-4">
          ...
          {speaker && (
            <div className="flex items-center text-sm text-gray-500">
              <svg className="w-4 h-4 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
              {speaker}
            </div>
          )}
          {price !== undefined && (
            <div className="flex items-center text-sm text-gray-500">
              <svg className="w-4 h-4 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1" /></svg>
...
const Activities = () => {
  const [events, setEvents] = useState([
    {
      ...
      status: "closed", // Sudah diset CLOSED agar turun ke bawah
    ...
    },
    {
      ...
      status: "upcoming", // Upcoming akan NAIK ke atas
      registrationUrl:null,
      registrationDeadline: null,
      capacity: null,
      price:"-",
  ...
  // --- FILTER & SORT LOGIC (PERBAIKAN UTAMA DISINI) ---
  const filteredEvents = events
    ...
    .sort((a, b) => {
      // Logic: Ongoing (1) > Upcoming (2) > Closed (3) > Completed (4)
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      .animate-in { opacity: 1 !important; transform: translateY(0) translateX(0) scale(1) rotateX(0) !important; }
  ...
  useEffect(() => {
    const resetAnimations = () => {
      ...
      elements.forEach(el => el.classList.remove('animate-in', 'animate-out'));
    };
  ...
  }, [filterStatus, filterCategory, filteredEvents.length]);
  ...
  return (
    <section className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-16 sm:py-24">
          <div className="text-center">
            <div data-aos="fade-down" className="inline-flex items-center px-4 py-2 rounded-full bg-blue-100 text-blue-800 text-sm font-medium mb-6">
            ...
            <h1 data-aos="zoom-in" data-aos-delay="200" className="text-4xl sm:text-5xl font-bold text-gray-900 mb-4">
              Kegiatan <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Kami</span>
            </h1>
            <p data-aos="fade-up" data-aos-delay="400" className="text-xl text-gray-600 max-w-2xl mx-auto leading-relaxed">
      ...
      <div className="max-w-7xl mx-auto px-6 mb-8">
        <div data-aos="fade-up" className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1">
              <label className="block text-sm font-medium text-gray-700 mb-2">Filter Status</label>
              <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            ...
            <div className="flex-1">
              ...
              <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            ...
            <div className="flex items-end">
              <button onClick={() => { setFilterStatus('all'); setFilterCategory('all'); }} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors duration-200 text-sm font-medium">
                Reset Filter
              </button>
            </div>
          </div>
          <div className="mt-4 text-sm text-gray-600">Menampilkan {filteredEvents.length} dari {events.length} kegiatan</div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 pb-16">
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {filteredEvents.map((event, index) => (
            <div key={event.id} data-aos="fade-up" data-aos-delay={index * 150}>
              <EvenCard {...event} className="transform hover:scale-105 transition-all duration-300 h-full" />
            </div>
          ))}
        </div>
        ...
        <div data-aos="fade-up" data-aos-delay="200" className="mt-16 text-center">
          <div className="inline-flex flex-col sm:flex-row gap-4">
            <button data-aos="slide-up" data-aos-delay="400" className="px-8 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold rounded-full hover:from-blue-700 hover:to-indigo-700 transform hover:scale-105 transition-all duration-200 shadow-lg hover:shadow-xl" onClick={() => window.open('https://www.instagram.com/penalaranudinus/', '_blank')}>
        ...
        {/* Stats Section */}
        <div data-aos="flip-up" data-aos-delay="300" className="mt-20 bg-white rounded-3xl shadow-xl p-8 border border-gray-100">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div
                ...
                className={stat.isFirst ? "text-center" : "text-center p-6 rounded-2xl bg-white/60 backdrop-blur-sm border border-white/20"}
              >
                {stat.isFirst ? (
                  <>
